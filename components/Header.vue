  ...
  >
    <div class="w-full h-full flex items-center justify-between px-4 md:px-8">
      ...
      >
        ...
        <div
          class="relative"
          @mouseenter="isListsDropdownOpen = true"
          @mouseleave="isListsDropdownOpen = false"
        >
          <button
            class="text-xs uppercase tracking-wider hover:text-[#64748b] nav-link flex items-center gap-1"
          ...
          >
            <div
              v-show="isListsDropdownOpen"
              class="absolute top-full right-0 mt-2 bg-white border border-gray-200 rounded shadow-lg min-w-[120px] z-[60]"
            >
              <NuxtLink
                to="/reading"
                class="block px-4 py-2 text-xs uppercase tracking-wider hover:bg-gray-50 hover:text-[#64748b]"
                :class="{ 'font-bold': isActive('/reading') }"
                >Reading</NuxtLink
              >
            </div>
      ...
      <div class="flex flex-col py-4 px-4 space-y-4">
        ...
        <div>
          <button
            ...
            class="text-xs uppercase tracking-wider hover:text-[#64748b] flex items-center gap-1 w-full text-left"
            :class="{ 'font-bold': isActive('/reading') }"
          >
            Lists
            <svg
          ...
          >
            <div v-show="isListsDropdownOpen" class="pl-4 mt-2 space-y-2">
              <NuxtLink
                to="/reading"
                class="block text-xs uppercase tracking-wider hover:text-[#64748b] nav-link"
                :class="{ 'font-bold': isActive('/reading') }"
                @click="isMenuOpen = false"
                >Reading</NuxtLink
              >

<template>
  <li class="flex items-baseline py-3">
    ...
    <span class="font-garamond italic text-[15px] md:text-[16px] text-gray-600 shrink-0 inline-flex items-baseline gap-3">
      <span>{{ author }}</span>
      <template v-if="stars">
        <span class="not-italic tracking-[0.05em] text-black">{{ starGlyphs }}</span>
      </template>
